        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if button_restart.collidepoint(event.pos):
                    ...
                    exit()
def main():
    cndrur_img = pygame.image.load("bombb.png").convert_alpha()
    ...
    while not game_started and running:
        ...
        screen.fill((0, 0, 0))
        screen.blit(bg_img, (0, bg_img_y - size[1]))
        screen.blit(bg_img, (0, bg_img_y))
        time_ms = pygame.time.get_ticks()
        angle = (time_ms / 5) % 360
        rotated_img = pygame.transform.rotozoom(cndrur_img, angle, 1.0)
        center_y = 150 + math.sin(time_ms / 500) * 10  # 위아래 흔들림
        rotated_rect = rotated_img.get_rect(center=(size[0] // 2, size[1] // 2))

        screen.blit(rotated_img, rotated_rect)
        #screen.blit(cndrur_img, (40, 400, 40, 400))
        title_font = pygame.font.SysFont("malgungothic", 30)
        title = title_font.render("날아라 김 모 씨", True, (255, 255, 255))
    ...
    while True:
        ...
        while MainEv == 0:
            ...
            if shop_open:
                ...
                item5 = font.render(f"[5] 보호막 (1000G) | {shield_count}개 작착중", True, item_color(shield_count > 0))
            ...
            for i in range(len(bombList)):
                for j in range(len(enList)):
                    ...
                    if crash(bomb, enm):
                        ...
                        if enm.hp <= 0:
                            denmList.append(j)
                            gained_gold = random.randrange(1, 100) #골드!
                            gold += gained_gold
                            kill_count += 1
